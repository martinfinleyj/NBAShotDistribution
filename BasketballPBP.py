...
## SHOT DISTRIBUTION TESTING ##

'''
total_rows = len(shots_player)
inside_paint_dist = sum(shots_player["inside_paint"])
midrange_dist = sum(shots_player["midrange"])
above_break_3_dist = sum(shots_player["above_break_3"])
corner_3_dist = sum(shots_player["corner_3"])

inside_paint_pct = sum((shots_player["inside_paint"]) & (shots_player["made"] == 1)) / sum(shots_player["inside_paint"])
midrange_pct = sum((shots_player["midrange"]) & (shots_player["made"] == 1)) / sum(shots_player["midrange"])
above_break_3_pct = sum((shots_player["above_break_3"]) & (shots_player["made"] == 1)) / sum(shots_player["above_break_3"])
corner_3_pct = sum((shots_player["corner_3"]) & (shots_player["made"] == 1)) / sum(shots_player["corner_3"])

inside_paint_ast = sum((shots_player["inside_paint"]) & (shots_player["assisted"] == True)) / sum(shots_player["inside_paint"])
midrange_ast = sum((shots_player["midrange"]) & (shots_player["assisted"] == True)) / sum(shots_player["midrange"])
above_break_3_ast = sum((shots_player["above_break_3"]) & (shots_player["assisted"] == True)) / sum(shots_player["above_break_3"])
corner_3_ast = sum((shots_player["corner_3"]) & (shots_player["assisted"] == True)) / sum(shots_player["corner_3"])


data = {
    
    "shot type": ["inside_paint", "midrange", "above_break_3", "corner_3"],
    "distribution": [inside_paint_dist/total_rows, midrange_dist/total_rows, above_break_3_dist/total_rows, corner_3_dist/total_rows],
    "shot count": [inside_paint_dist, midrange_dist, above_break_3_dist, corner_3_dist],
    "shot percent": [inside_paint_pct, midrange_pct, above_break_3_pct, corner_3_pct],
    "expected points": [inside_paint_pct * 2, midrange_pct * 2, above_break_3_pct * 3, corner_3_pct * 3],
    "ast percent": [inside_paint_ast, midrange_ast, above_break_3_ast, corner_3_ast]
    
    }

shot_distribution = pd.DataFrame(data)


## OPTIMIZATION FRAMEWORK TESTING ## 


c = [-inside_paint_pct * 2, -midrange_pct * 2, -above_break_3_pct * 3, -corner_3_pct * 3]

A_eq = [
    [1, 1, 1, 1]
]

b_eq = [total_rows] # limits to x total shots by player

bounds = [
    (inside_paint_dist-20, inside_paint_dist+20),  # Paint
    (midrange_dist-20, midrange_dist+20), # Midrange
    (above_break_3_dist-20, above_break_3_dist+20),  # Above-break 3 
    (corner_3_dist-20, corner_3_dist+20),  # Corner 3  
]
...
print(data["shot count"])

optimized_points = (
    result.x[0] * inside_paint_pct * 2 +
    result.x[1] * midrange_pct * 2 +
    result.x[2] * above_break_3_pct * 3 +
    result.x[3] * corner_3_pct * 3
)

print(sum((shot_distribution["shot count"]) * shot_distribution["expected points"])/100)
print(optimized_points/100)
print(optimized_points/100 - sum((shot_distribution["shot count"]) * shot_distribution["expected points"])/100)
'''